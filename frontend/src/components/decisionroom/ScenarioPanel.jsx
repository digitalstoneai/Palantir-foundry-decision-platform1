export default function ScenarioPanel({ events, selectedEventId, onSelectEvent, options }) {
  return (
    <div className="panel">
      ...
      >
        ...
        {events.map((evt) => (
          <option key={evt.id} value={evt.id}>
            [{evt.severity}] {evt.description.slice(0, 60)}
          </option>
        ))}
      </select>

      <h3>Response Options</h3>
      {!options.length && <p style={{ color: 'var(--text-muted)' }}>No options for this event.</p>}
